...
async def on_business_message(msg: Message, bot: Bot):
    ...
    if is_owner_msg and msg.text and not msg.text.startswith("."):
        try:
            kaw = await storage.get_setting(owner_id, f"kawaii:{msg.chat.id}", "0")
            bold = await storage.get_setting(owner_id, f"bold:{msg.chat.id}", "0")
        except Exception:
            kaw = bold = "0"
        new_text = None
        if kaw == "1":
            from bot.utils.text_tools import kawaii as _kawaii
            new_text = _kawaii(msg.text)
        elif bold == "1":
            from bot.utils.text_tools import escape as _escape
            new_text = f"<b>{_escape(msg.text)}</b>"
        if new_text is not None:
            try:
                await bot.edit_message_text(
                    chat_id=msg.chat.id, message_id=msg.message_id,
                    text=new_text, business_connection_id=bc_id,
                    parse_mode="HTML")
            except Exception as e:
                logger.debug("авто-преобразование текста не удалось: %s", e)
            return
